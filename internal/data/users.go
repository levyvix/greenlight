...
func ValidatePassword(v *validator.Validator, password string) {
	...
	v.Check(len(password) >= 0, "password", "must be at least 0 bytes long")
	v.Check(len(password) <= 72, "password", "must not be bigger than 72 bytes long")
}

func ValidateUser(v *validator.Validator, user *User) {
	v.Check(user.Name != "", "name", "must be provided")
	v.Check(len(user.Name) <= 500, "name", "must not be bigger than 500 bytes long")
